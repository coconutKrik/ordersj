      ...
      <ul class="cart_list_td clearfix" v-for="(dishes, i) in cart" :key="i">
        ...
        <li class="col06">
          <div class="num_add">
            ...
            <input
              v-model="dishes.count"
              @focus="origin_input = dishes.count"
              @blur="on_input(i)"
              type="text"
              class="num_show fl"
            />
      ...
      <ul class="settlements">
        <li class="col01">
          <input
            type="checkbox"
            name=""
            @change="on_selected_all"
            v-model="selected_all"
          />
        </li>
        <li class="col02">全选</li>
        <li class="col03">
          合计(不含运费)：<span>¥</span><em>{{ total_selected_amount }}</em
  ...
  computed: {
    total_count: function () {
      ...
      for (var i = 0; i < this.cart.length; i++) {
        ...
        this.cart[i].amount = (this.cart[i].price * this.cart[i].count).toFixed(
          2
        );
      }
      return total;
